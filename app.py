...
def safe_json_load(filepath):
    """Load only the first JSON object from the file."""
...
def show_section(section_id):
    ...
    data = safe_json_load('requirements.json')
    chapters = data.get('DNV0145', {}).get('chapters', [])

    section = next((c for c in chapters if c['chapterTitle'].startswith('Section 10')), None)
    if not section:
        return "Section 10 not found", 404
